...
from .appearance_net import Appearance_Adapter
...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.appearance_adapter = Appearance_Adapter(cin=int(3 * 64), channels=[320, 640, 1280, 1280][:4], nums_rb=2, ksize=1, sk=True, use_conv=False)
    ...
    def set_attn_processor(
        ...
        def fn_recursive_attn_processor(name: str, module: torch.nn.Module, processor):
            ...
            for sub_name, child in module.named_children():
                if ("temporal_transformer" not in sub_name) and ("motion_modules" not in sub_name):
                    print(sub_name)
                    fn_recursive_attn_processor(f"{name}.{sub_name}", child, processor)

        for name, module in self.named_children():
            if "temporal_transformer" not in name or "motion_modules" not in name:
    ...
    def forward(
        ...
        ref_images: Optional[torch.FloatTensor] = None,
        ...
        if pose_cond_fea is not None:
            sample = sample + pose_cond_fea * pose_scale

        # down
        down_block_res_samples = (sample,)
        appearance_features = self.appearance_adapter(ref_images)

        for idx, downsample_block in enumerate(self.down_blocks):
            appearance_feature = appearance_features[idx]
            if (
